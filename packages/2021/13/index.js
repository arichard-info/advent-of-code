...
const parseData = (str) => {
    [rawCoordinates, rawInstructions] = str.split(EOL + EOL);
    const instructions = rawInstructions.split(EOL).map(instruction => {
    ...
    return {
        coordinates: rawCoordinates.split(EOL).map(pair => pair.split(',').map(Number)),
        instructions
    }
}
...
const drawCoordinates = (coordinates) => {
    ...
    return arr.map(row => row.join('')).join(EOL);
}
